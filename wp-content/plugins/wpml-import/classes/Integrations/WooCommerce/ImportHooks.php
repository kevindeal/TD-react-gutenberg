...
abstract class ImportHooks implements \IWPML_Action {
	...
	private $relativeCommaFields = [
		// The key 'children' becomes 'children' when processing SKUs to IDs.
		'children'       => 'Grouped products',
		'upsell_ids'     => 'Upsells',
	...
	protected function manageRelatedProducts( $data, $language ) {
		array_walk( $this->relativeFields, function( $keyInCsv, $keyInData ) use ( &$data, $language ) {
		...
		array_walk( $this->relativeCommaFields, function( $keyInCsv, $keyInData ) use ( &$data, $language ) {
			if ( $this->hasRelatedFieldValue( $keyInCsv ) ) {

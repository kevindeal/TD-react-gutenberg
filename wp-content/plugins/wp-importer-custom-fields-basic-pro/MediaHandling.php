...
class MediaHandling{
	...
	public function image_import($data_array,$check,$mode,$line_number,$unikey_value ,$unikey_name,$poly_values){
		...
		elseif($media_handle['media_settings']['large'] == 'true'){
			...
			if($check == 'Filename'){
				...
				if(!empty($poly_values)){
					...
					$attachment_id=$wpdb->get_var("SELECT p.ID FROM {$wpdb->prefix}posts as p inner join {$wpdb->prefix}term_relationships as tr ON tr.object_id=p.ID inner join {$wpdb->prefix}term_taxonomy as tax on tax.term_taxonomy_id=tr.term_taxonomy_id inner join {$wpdb->prefix}terms as t on t.term_id=tax.term_id  where tax.taxonomy ='language'  and t.slug='$language_code' and p.post_type = 'attachment'  AND p.guid LIKE '%$fimg_names' order by ID DESC ");
				}else{
				...
				$post_info = array(
					...
					if(!empty($poly_values)){
						$language_code = $poly_values['language_code'];
						$check_for_existence=$wpdb->get_var("SELECT p.ID FROM {$wpdb->prefix}posts as p inner join {$wpdb->prefix}term_relationships as tr ON tr.object_id=p.ID inner join {$wpdb->prefix}term_taxonomy as tax on tax.term_taxonomy_id=tr.term_taxonomy_id inner join {$wpdb->prefix}terms as t on t.term_id=tax.term_id  where tax.taxonomy ='language'  and t.slug='$language_code' and p.post_type = 'attachment' AND p.ID =$id order by ID DESC ");
					}else{
		    ...
		    // else if($check == 'Featured_image'){
				...
				if(!empty($poly_values)){
					$language_code = $poly_values['language_code'];
					$get_result=$wpdb->get_results("SELECT p.ID FROM {$wpdb->prefix}posts as p inner join {$wpdb->prefix}term_relationships as tr ON tr.object_id=p.ID inner join {$wpdb->prefix}term_taxonomy as tax on tax.term_taxonomy_id=tr.term_taxonomy_id inner join {$wpdb->prefix}terms as t on t.term_id=tax.term_id  where tax.taxonomy ='language'  and t.slug='$language_code' and p.post_type = 'attachment' AND p.guid LIKE '$guid2%' order by ID DESC ");
				}
				else{

...
class ImageSchedule {
    ...
    public function image_schedule($schedule_array,$unikey,$module,$clikey)
    {
        ...
        $get_result = $wpdb->get_results("SELECT  post_id FROM {$wpdb->prefix}ultimate_cf_importer_shortcode_manager WHERE status = 'pending' ORDER BY post_id ASC LIMIT 5", ARRAY_A);
        if(empty($get_result)){
            ...
            foreach ($records as $title => $id) { 
                $weight = 0;
                $get_post_id = $wpdb->get_var("SELECT post_id FROM {$wpdb->prefix}ultimate_cf_importer_shortcode_manager WHERE post_id = $id AND $unikey = '$schedule_array' AND status = 'pending' ");
                $get_image_meta = $wpdb->get_var("SELECT image_meta FROM {$wpdb->prefix}ultimate_cf_importer_shortcode_manager WHERE post_id = $id AND $unikey = '$schedule_array' AND status = 'pending' ");  
                ...
                elseif( strpos($get_shortcode, 'pods_image__') !== false ){
                    //fetch import type
                    $importazValue =  $wpdb->get_var($wpdb->prepare("select post_type from {$wpdb->prefix}posts where ID = %d", $get_post_id));
                    $get_original_image = [];
                    $get_shortcode = esc_sql($get_shortcode);
                    $get_original_image = $wpdb->get_results("SELECT original_image FROM {$wpdb->prefix}ultimate_cf_importer_shortcode_manager WHERE post_id = $id AND image_shortcode = '{$get_shortcode}' AND status = 'pending' ");
                    ...
                    $my_attach_id=[];
                    foreach($get_original_image as $get_original_image_value){;
                        $original_image_url = $get_original_image_value->original_image;
                        $my_attach_id[] = self::$media_instance->media_handling( $original_image_url, $id, array(),'','pods',$hash_key,$templatekey);
                    }
                    foreach($my_attach_id as $attach_id){
                        if(!empty($attach_id)){
                          //  update_post_meta($id, $get_image_fieldname[1], $attach_id);
                            $pod_id =array(intval($attach_id));
                            ...
                            $importazValue = esc_sql($importazValue);
                            $pods_id = $wpdb->get_results("SELECT ID FROM {$wpdb->prefix}posts WHERE post_name = '{$importazValue}' AND post_type = '_pods_pod'");          
    
                                $related_file_id = $attach_id;
                                $pod = pods($importazValue);
                        
                                $field_info = $pod->fields($get_image_fieldname[1]);
                                $field_id = $field_info['id'];
                                $check_result = $wpdb->get_results($wpdb->prepare("SELECT ID,pod_id,field_id,item_id,related_item_id FROM {$wpdb->prefix}podsrel WHERE pod_id = %d AND item_id = %d AND field_id = %d",$pods_id[0]->ID,$id,$field_id));
                                if(empty($check_result) || isset($check_result)){   
                                    $pods_rel_table = $wpdb->prefix . "podsrel";
                                        ...
                                        $pods_rel_table = $wpdb->prefix . "podsrel";
                                        $my_result = $wpdb->insert($pods_rel_table, array('pod_id' => $pods_id[0]->ID,'field_id' => $field_id,'item_id' => $id,'related_pod_id' => 0,'related_field_id' =>0,'related_item_id' => $related_file_id,'weight' => $weight));
                                        if($weight >= 0){
                                            $weight++;
                                        }
    ...
    public function jetengine_image_update($id, $get_shortcode, $get_image_meta, $image_shortcode,$hash_key,$templatekey){
        ...
        elseif($image_shortcode == 'jetengine_repeater_media_'){
            ...
            foreach($get_original_image as $gallery_image){
                $get_origin_image = $gallery_image['original_image'];
                $shortcode_rep = $wpdb->get_var("SELECT image_shortcode FROM {$wpdb->prefix}ultimate_cf_importer_shortcode_manager WHERE original_image ='$get_origin_image' AND post_id = $id AND status = 'pending' ");
                ...
                if($attach_id){
                    ...
                    elseif(in_array($type,$listTaxonomy)){
                        ...
                        update_post_meta($id, $rep[3], $get_value);  
                    }
        ...
        elseif($image_shortcode == 'jetenginecpt_repeater_media_'){
            ...
            foreach($get_original_image as $gallery_image){
                ...
                if($attach_id){
                    ...
                    update_post_meta($id, $rep[2], $get_value);  
    ...
    public function acf_gallery_image_update($id, $get_shortcode, $get_image_meta, $image_shortcode, $get_import_type,$hash_key, $templatekey){
        ...
        foreach($get_gallery_images as $gallery_image){
            ...
            $get_origin_img = $gallery_image['original_image'];
            if($attach_id){ 
                $gallery_ids[] = $attach_id;
                $this->update_status_shortcode_table($id, $get_shortcode, $get_origin_img,'completed');
            }
            else{
                $this->update_status_shortcode_table($id, $get_shortcode,$get_origin_img, 'failed');
            }
        } 

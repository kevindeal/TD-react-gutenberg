...
class CoreFieldsImport
{
	...
	function set_core_values($header_array, $value_array, $map, $type, $mode, $line_number, $unmatched_row, $check, $hash_key, $acf, $pods, $toolset, $jetengine, $metabox, $update_based_on, $gmode, $templatekey,$poly_array=null)
	{
		...
		if (($type == 'Categories') || ($type == 'Tags') || ($type == 'Taxonomies') || ($type == 'Comments') || ($type == 'Users') || ($type == 'Customer Reviews') || ($type == 'nav_menu_item') || ($type == 'widgets')) {
			...
			if (($type == 'Categories') || ($type == 'Tags') || ($type == 'Taxonomies')) {
				$result = $taxonomies_instance->taxonomies_import_function($post_values, $mode, $import_type, $unmatched_row, $check, $hash_key, $line_number, $header_array, $value_array, $gmode, $templatekey,$poly_values);
			}
			if ($type == 'Users') {
			...
			if (isset($post_values['featured_image'])) {
				if (!empty($post_id) && !is_plugin_active('featured-image-from-url/featured-image-from-url.php')) {
			...
			if (isset($this->detailed_log[$line_number]) && is_array($this->detailed_log[$line_number]) && isset($this->detailed_log[$line_number]['Message'])) {
		...
		if ($optional_type == $type) {
			...
			if ($type == 'Images') {
				...
				$result = CoreFieldsImport::$media_instance->image_import($post_values, $check, $mode, $line_number, $unikey_value, $unikey_name,$poly_values);
			}
			...
			if ($check == 'ID') {
				...
				if(!empty($poly_values)){
					...
					$get_result=$wpdb->get_results("SELECT p.ID FROM {$wpdb->prefix}posts as p inner join {$wpdb->prefix}term_relationships as tr ON tr.object_id=p.ID inner join {$wpdb->prefix}term_taxonomy as tax on tax.term_taxonomy_id=tr.term_taxonomy_id inner join {$wpdb->prefix}terms as t on t.term_id=tax.term_id  where tax.taxonomy ='language'  and t.slug='$language_code' and p.ID='$ID' AND p.post_status != 'trash' AND p.post_type = '$import_as'");
				}
				else{
			...
			if (!in_array($check, $update)) {
				if ($update_based_on == 'acf') {
					...
					if (is_plugin_active('jet-engine/jet-engine.php')) {
						if (is_array($jetengine)) {

							foreach ($jetengine as $jetengine_key => $jetengine_value) {

								if (strpos($custom_value, '{') !== false && strpos($custom_value, '}') !== false) {
									$custom_value = $custom_key;
								}
								if ($jetengine_key == $check) {
									$get_key = array_search($jetengine_value, $header_array);
								}
								if (isset($value_array[$get_key])) {
									$csv_element = $value_array[$get_key];
								}
				...
				} else if ($update_based_on == 'metabox') {
					if (is_plugin_active('meta-box/meta-box.php')) {
						if (is_array($metabox)) {

							foreach ($metabox as $metabox_key => $metabox_value) {

								if (isset($custom_value) && strpos($custom_value, '{') !== false && strpos($custom_value, '}') !== false) {
									$custom_value = $custom_key;
								}
								if ($metabox_key == $check) {
									$get_key = array_search($metabox_value, $header_array);
								}
								if (isset($value_array[$get_key])) {
									$csv_element = $value_array[$get_key];
								}
			...
			if ($mode == 'Insert') {
				if(strpos($post_values['post_title'],'&') !== false){
					$post_values['post_title'] = str_replace('&','&amp;',$post_values['post_title']);
				}
			...
			if ($mode == 'Import-Update' || $mode == 'Update') {
				if (!empty($this->detailed_log) && isset($this->detailed_log[$line_number]) && preg_match("(Skipped)", $this->detailed_log[$line_number]['Message']) != 0) {
					...
					if (is_array($get_result) && !empty($get_result)) {
						...
						if(!empty($content)){
							if($generated_content == 401) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create Content. Invalid API key provided. Please check your API key.";	
							}
							else if($generated_content == 429) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create Content. Rate limit reached for requests or You exceeded your current quota.";	
							}
							else if($generated_content == 400) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create Content. Please check your Inputs.";	
							}
							else if($generated_content == 500) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create Content. The server had an error while processing your request.";	
							}
							else if($generated_content == 503) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create Content. The engine is currently overloaded, please try again later.";	
							}
							else if($generated_short_description == 401) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create short description. Invalid API key provided. Please check your API key.";	
							}
							else if($generated_short_description == 429) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create short description. Rate limit reached for requests or You exceeded your current quota.";	
							}
							else if($generated_short_description == 400) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create short description. Please check your Inputs.";	
							}
							else if($generated_short_description == 500) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create short description. The server had an error while processing your request.";	
							}
							else if($generated_short_description == 503) {
								$this->detailed_log[$line_number]['Message'] = 'Updated ' . $post_values['post_type'] . ' ID: ' . $post_id . ', ' . $post_values['specific_author']. 	"<b style='color: red;'> Notice : </b>Cannot create short description. The engine is currently overloaded, please try again later.";	
							}
							else{
	...
	function image_handling($id, $hash_key, $module, $templatekey)
	{
		...
		if (is_array($attach_id)) {
			...
			$wpdb->update(
				$wpdb->prefix . 'ultimate_cf_importer_shortcode_manager',
				array(
					'status' => 'Completed',
				),
				array(

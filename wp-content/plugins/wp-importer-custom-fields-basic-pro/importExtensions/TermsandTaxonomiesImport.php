...
class TermsandTaxonomiesImport {
	...
	function set_terms_taxo_values($header_array ,$value_array , $map, $post_id , $type, $mode , $line_number,$poly_array){
		$post_values = [];
			...
			case 'post_tag' :
				...
				if($mode == 'Import-Update' || $mode == 'Update'){
					...
					if(isset($tags [$termKey]) && $tags [$termKey] != '')
						$this->assignTermsAndTaxonomies($tags, $tag_name, $pID,$poly_values);
					if(preg_match("(Can't|Skipped|Duplicate)", $core_instance->detailed_log[$line_number]['Message']) === 0) {  

...
class TaxonomiesImport {
	...
	public function taxonomies_import_function ($data_array, $mode, $importType , $unmatched_row, $check , $hash_key , $line_number ,$header_array ,$value_array,$gmode,$templatekey,$poly_values) {
		...
		if($_display_type){
			...
			if(!empty($term_id_value[0]->term_id)){
				...
				$term_display_type_value = $wpdb->get_results("SELECT meta_key FROM {$wpdb->prefix}termmeta WHERE term_id = '$term_id_val'");
				if(isset($term_display_type_value) && is_array($term_display_type_value) && count($term_display_type_value) > 0){
					if (isset($term_display_type_value[0]->display_type)) {

...
class ImportHelpers
{
	...
	public function update_count($unikey_value)
	{
		$response = [];
		global $wpdb;
		$log_table_name = $wpdb->prefix . "cfimport_detail_log";
		$get_data =  $wpdb->get_results("SELECT skipped , created , updated FROM $log_table_name WHERE hash_key = '$unikey_value' ");

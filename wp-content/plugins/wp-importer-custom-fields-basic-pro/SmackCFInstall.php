...
class SmackCSVInstall {
	...
	public static function plugin_row_meta( $links, $file ) {
			$row_meta = array(
				'settings' => '<a href="' . esc_url( apply_filters( 'WP Importer Custom fields Pro_url', admin_url() . 'admin.php?page=com.smackcoders.csvimporternewcustom.menu&settings' ) ) . '" title="' . esc_attr( __( 'Visit Plugin Settings', 'wp-importer-customfields-pro' ) ) . '" target="_blank">' . __( 'Settings', 'wp-importer-customfields-pro' ) . '</a>',
	...
	public static function wp_ultimate_csv_importer_notice() {
		...
		if($get_notice != 'off' && isset($_REQUEST['page']) && in_array(sanitize_text_field($_REQUEST['page']), $smack_uci_pages)) {
			...
			<div class='notice updated uci-message wc-connect is-dismissible' onclick="dismiss_notices('upgrade_notice');">
				<?php
				if ( get_option( 'ULTIMATE_CSV_IMP_VERSION' ) == '7.0.4' ) {
					?>

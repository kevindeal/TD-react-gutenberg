...
class SaveMapping{
		...
		if(array_key_exists('CORECUSTFIELDS',$mapdata)){
			...
			foreach($custfields as $cust_key => $cust_val){
				if(is_array($cust_val)){
					$mapdata['CORECUSTFIELDS'][$cust_val['name']] = $cust_val['value'];
	...
	public function save_fields_function() {
		...
		if(array_key_exists('CORECUSTFIELDS',$mapped_fields)){
			...
			foreach($custfields as $cust_key => $cust_val){
				if(is_array($cust_val)){
					$mapped_fields['CORECUSTFIELDS'][$cust_val['name']] = $cust_val['value'];
		...
		$result['success'] = true;
		if(isset($image_included)){
			$result['image_included']=$image_included;
			}		
		if(isset($fileiteration)){
	...
	public function main_import_process($map , $header_arrays ,$value_array , $selected_type , $get_mode, $line_number , $unmatched_row, $check , $hash_key, $update_based_on, $gmode, $templatekey = null){
		...
		foreach($map as $group_name => $group_value){
			if($group_name == 'CORE'){
				...
				$jetengine_map = isset($map['JE']) ? $map['JE'] : '';
				...
				$core_instance = CoreFieldsImport::getInstance();
				$post_id = $core_instance->set_core_values($header_array ,$value_array , $map['CORE'] , $selected_type , $get_mode, $line_number , $unmatched_row, $check , $hash_key, $acf_map, $pods_map ,$types_map,$jetengine_map,$metabox_map, $update_based_on, $gmode,$templatekey,$poly_map);		
			}
			else{				
				$acf_map = isset($map['ACF']) ? $map['ACF'] : '';
				$jetengine_map = isset($map['JE']) ? $map['JE'] : '';
        ...
        if(!empty($post_id)){
			foreach($map as $group_name => $group_value){
				...
				case 'ACF':	
					...
					$acf_pro_instance->set_acf_pro_values( $header_array, $value_array, $map['ACF'],$acf_image , $post_id, $selected_type ,$get_mode,$line_number,$hash_key,$gmode,$templatekey,$poly_array);
					break;
				...
				case 'NEXTGEN':
					SaveMapping::$nextgen_instance->nextgenImport($header_array ,$value_array , $map['NEXTGEN'], $post_id , $selected_type,$hash_key);
					break;
				...
				case 'POLYLANG':
                    ...
                    $polylang_instance->setPolylangValues($header_array, $value_array, $map['POLYLANG'], $post_id, $selected_type, $get_mode);
                    break;	
		...
		foreach($keydata as $widgetname) {
			...
			switch($widgetname) {
				...
				case 'STMQUIZ':
					...
					break;	
				case 'STMQUESTION':
					$fieldtype = "question_settings_fields_stm";
					break;	
				case 'STMORDER':	
			...
			foreach($map[$widgetname] as $key => $value){									
					if( strpos($key,'->static') !== false || strpos($key,'->math') !== false || strpos($key,'->cus1') !== false || strpos($key,'->cus2') !== false || strpos($key,'->openAI') !== false || strpos($key,'->num') !== false) {
						
					}
					else{					
						if(!empty($newmap) && !in_array($key,$newmap) && $widgetname !='CORECUSTFIELDS'){

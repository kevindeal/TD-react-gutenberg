...
$heroText = get_field('resources_header');
        ...
        <div class="featured-inner container mx-auto">
            <span class="text-3xl block mb-[30px]">Featured Resource</span>
				...
				<div class="featured-post grid grid-cols-12 gap-6 items-center">
                    ...
                    <?php
                      $featured_resource_id = get_post_meta(get_the_ID(), 'featured_resource', true);
          ...
          ?>
					...
					<a class="btn-primary text-white inline-block" href="<?php echo trailingslashit(get_permalink($featured_resource->ID)); ?>">Read post</a>
                    </div>
                    <div class="featured-right sm:col-span-5 col-span-12 overflow-hidden">
                        <a href="<?php echo trailingslashit(get_permalink($featured_resource->ID)); ?>">
        ...
        <?php
            ...
            if ($query->have_posts()) {
                ...
                while ($query->have_posts()) {
                    ...
                    $resource_type = $resource_types ? array_pop($resource_types)->slug : null;
                    $card_type = $resource_type ? $resource_type : get_post_type();
                    get_template_part("template-parts/cards/card", $card_type);
                    /*
                    $external_link = get_field('external_link', get_the_ID());
                    echo '<div class="relative inline-table mb-[45px] overflow-hidden">'; 
                    ...
                    if (has_post_thumbnail()) {
                        ...
                        echo '<a href="' . ($external_link ? $external_link : trailingslashit(get_permalink()) ). '" target="'.($external_link ? '_blank' : '').'" style="background-image: url(' . $thumbnail_url . '); display: block; background-size: cover; background-position: center; padding-bottom: 100%; border-radius: 20px;
                    ...
                    echo '<h4 class="mb-[10px] font-medium"><a class="text-3xl no-underline leading-[1.45] line-clamp-2 lg:line-clamp-none" href="' . ($external_link ? $external_link : trailingslashit(get_permalink()) ) . '" target="'.($external_link ? '_blank' : '').'">' . get_the_title() . '</a></h4>';

...
<?php
  ...
  $block_id = array_key_exists("anchor", $block) ? $block["anchor"] : $scope_id;
?>
<div id="<?php print $block_id ?>" data-scope="<?php print $scope_id ?>" class="container mx-auto color-variant p-4 lg:p-[50px] <?php if ($faqData['color_variation'] == 'Dark') {
    echo 'bg-primaryBlue-100 text-white';
} else {
    echo 'bg-white';
} ?>
<?php if ($layout_variant == '2col') {
  ...
  <h2 class="title font-header text-4xl lg:pb-[50px]"><?php echo $faqData['first_tab_title'] ?></h2>
  <div class="layout-variant <?php if ($faqData['layout_variant'] == '1col') {
    echo 'container mx-auto';
  } else {
      echo 'w-3/4';
  }
          ...
          <div class="accordion_title <?php 
            ...
            <?php
              ...
              if (!empty($section['section_questions'])) {
                ...
                <div class="accordion_content  border-solid border-t border-white-300 transition-all duration-300 ease-in-out py-[32px]">
                  <div class="question_text pb-5">
                    <?php if (!empty($question_text)) : ?>
                      <h4 class="font-display text-l">
